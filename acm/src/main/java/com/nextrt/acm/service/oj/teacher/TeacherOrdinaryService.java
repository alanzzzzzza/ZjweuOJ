...
@Service
public class TeacherOrdinaryService {
    @Autowired
    ...
    public Result deleteUserJoin(int userId, int contestId, int delUserId) {
        Contest old = contestBiz.getContestById(contestId);
        if (old.getUserId() != userId) return new Result(-1, "权限不足，请确认该竞赛创建人是本帐号");
        if (joinInfoBiz.deleteByContestIdAndUserId(delUserId, contestId) > 0) {
            submissionBiz.deleteSubmissionByContestIdAndUserId(contestId, delUserId);//删除提交数据
            resultBiz.deleteByContestIdAndUserId(contestId, delUserId);//删除问题结果
            reportIPBiz.deleteByContestIdAndUserId(contestId, delUserId);//删除竞赛IP上报数据
            return new Result(1, "删除成功");
        } else
            return new Result(-1, "删除失败");
    }

    public Result addJoinUser(String data, int type, int userId, int contestId) {
        Contest old = contestBiz.getContestById(contestId);
        if (old.getUserId() != userId) return new Result(-1, "权限不足，请确认该竞赛创建人是本帐号");
        User user = null;
        List<User> list = null;
